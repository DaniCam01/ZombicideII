...
public class Zombie implements Serializable  {
	...
	private int puerta, distancia, alturarandom, posinega;
	public int maxpaciencia=10;
	//private int speed;

	public Zombie(Player player, int velocidad) {  
		this.player = player;
		...
		alturarandom= (int) (Math.random()*50);
		posinega= (int) (Math.random()*2);
		if(puerta==0) {
			x = -200-distancia;
			if(posinega==0) 
				y = 700+alturarandom;
	...
	public boolean entraMapa() {
		Rectangle recZombie = new Rectangle(x , y, ANCHO, ALTO);
		Rectangle recBola = new Rectangle(150, 150, Constant.WIDTHSCREEN-300, Constant.HEIGHTSCREEN-300);
		return recZombie.intersects(recBola);
	}
